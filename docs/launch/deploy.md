...
documents their configuration and the verification required for the v0.2.0 release.
...
3. Smoke test: `curl https://sommelier-api-yd1m.onrender.com/health` returns status `ok`
   and contract `mln601-a2-v8`; open the live `/docs` and use "Try it out" on `/predict`.
...
```markdown
| **Sommelier API** | Two-lens wine quality predictor on 5,320 unique UCI wines after duplicate removal. FastAPI service serving a Random Forest regressor (R² 0.41) and the A2 v8 parity-locked, class-weighted Random Forest classifier (ROC-AUC 0.834, sensitivity 0.714, specificity 0.806), with a Streamlit tasting-room UI and a local/API inference toggle. | ✅ | [Repo](https://github.com/lfariabr/sommelier-api) | [App](https://sommelier-api.streamlit.app/) / [Article](https://dev.to/lfariaus/i-gave-the-same-6497-wines-to-two-models-and-asked-them-different-questions-4hdn) |
```

## 5. Publish the article
`docs/article.md` is the maintained source for the published dev.to article. Update the
published article after v0.2.0 so its model, metrics and trade-off match this repository.

## Smoke test (after each deploy)
- [ ] `/health` reports `mln601-a2-v8` and source commit `c5be26cf1bb7cc71f8f057fba45aa5b3ea8dd5b2`.
- [ ] `/model/info` reports `RandomForestClassifier` and the committed v8 metrics.
- [ ] `/openapi.json` reports API version `0.2.0`.
...
- [ ] Streamlit cold-loads, predicts in local mode and shows the v8 Model Card.
